    ...
    <aside
        ...
        <div class="flex gap-1 px-4 pb-2">
            ...
            <button wire:click="$set('activeTab','trash')"
                class="flex-1 py-1.5 text-xs font-medium rounded-lg transition-colors {{ $activeTab === 'trash'
                    ? 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400'
                    : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700' }}">🗑️
                Trash </button>
        </div>

        {{-- Tags filter --}}
        @if ($this->allTags->count() && $activeTab !== 'trash' && $activeTab !== 'daily')
            <div class="px-4 pb-2">
        ...
        {{-- Notes List --}}
        <div class="flex-1 overflow-y-auto px-3 pb-4 mt-1">
            @if ($activeTab === 'daily')
                @forelse($this->dailyNotes as $dateGroup => $dateNotes)
                    <div class="mb-4">
                        <p class="text-[10px] uppercase tracking-widest text-gray-400 font-semibold mb-2 px-0.5">
                            {{ $dateGroup }}</p>
                ...
                @empty
                    <div class="flex flex-col items-center justify-center py-12 text-center">
                        <svg class="w-10 h-10 text-gray-300 dark:text-gray-600 mb-3" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        <p class="text-sm text-gray-400 dark:text-gray-500">No notes today</p>
                        <button wire:click="createNote"
                            class="mt-3 text-xs text-indigo-500 hover:text-indigo-600 font-medium">
                            + Create a note
                        </button>
                    </div>
                @endforelse
        ...
        <div class="border-t border-gray-100 dark:border-gray-700 p-3 mt-auto">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center overflow-hidden flex-shrink-0 cursor-pointer hover:ring-2 hover:ring-indigo-500 transition-all"
                    @click="openImageZoom('{{ auth()->user()->profile_picture ? asset('storage/' . auth()->user()->profile_picture) : '' }}')"
                    title="Click to zoom">
    ...
    <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        @if ($activeNoteId)
                ...
                @if ($activeTab !== 'trash')
                    ...
                    <div class="relative" x-data="{ openTitleColor: false }">
                        ...
                        <div x-show="openTitleColor" @click.outside="openTitleColor = false" x-cloak
                            ...
                            @foreach (['#1f2937', '#374151', '#6b7280', '#ef4444', '#f97316', '#eab308', '#22c55e', '#3b82f6', '#8b5cf6', '#ec4899'] as $color)
                                <button wire:click="setTitleColor('{{ $color }}')"
                                    @click="openTitleColor = false"
            ...
            <div class="flex-1 overflow-y-auto"
                style="background-color: {{ $noteColor !== '#ffffff' ? $noteColor : '' }}"
                    ...
                    @if ($showDrawingCanvas && $activeTab !== 'trash')
                        <div class="mb-6 rounded-2xl overflow-hidden border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm"
                            ...
                            <canvas id="drawingCanvas" class="w-full touch-none cursor-crosshair bg-white"
                                style="height: 400px; display: block;" @mousedown="startDraw($event)"
                                @mousemove="draw($event)" @mouseup="stopDraw()" @mouseleave="stopDraw()"
                                @touchstart.prevent="startDraw($event.touches[0])"
                                @touchmove.prevent="draw($event.touches[0])" @touchend="stopDraw()">
                            </canvas>
                        </div>
                    @endif
                    ...
                    @if (count($images))
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
                            @foreach ($images as $index => $img)
                                <div class="relative group rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 shadow-sm aspect-video bg-gray-100 dark:bg-gray-800 cursor-pointer hover:ring-2 hover:ring-indigo-500 hover:ring-offset-2 dark:hover:ring-offset-gray-900 transition-all"
                                    @click="openImageZoom('{{ $img }}')" title="Click to zoom">
                                    ...
                                    @if ($activeTab !== 'trash')
                                        <button wire:click="removeImage({{ $index }})" @click.stop
        ...
        '#14b8a6' => 'Teal',
                                        ...
                                        <button onclick="applyTextColor('{{ $hex }}')"
                                            title="{{ $name }}"
